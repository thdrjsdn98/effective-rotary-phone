/* ============================================================
   ...
   반드시 STORAGE_PREFIX('electric_')로 시작합니다.
   다른 GitHub Pages 프로젝트(소방/타 전기과목 등)와
   동일 도메인에서 운영되어도 데이터가 섞이지 않도록
...
async function exportUserData() {
    // 백업 데이터에는 이 프로젝트(electric_) 소유 데이터만 포함합니다.
...
function importUserData(event) {
    ...
    reader.onload = function (e) {
        try {
            ...
            // 복원도 electric_ 네임스페이스 안에서만 수행 (다른 프로젝트 데이터 건드리지 않음)
...
// electric_ 로 시작하는 이 프로젝트 소유 키만 선별 삭제합니다.
